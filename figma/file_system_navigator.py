...
json_data = {
    "root": {
        ...
        "children": {
            ...
            "photos": {
                "type": "folder",
                "children": {"vacation.jpg": {"type": "file", "size": 2048}},
            },
...
class FileSystemNavigator:
    ...
    def add_file(self, folder_path, filename, size):
        ...
        folder_node = self.file_system_nodes[folder_name]
        if folder_node.type != "folder":
            print(f"{folder_name} is not a folder")
            return False
        
        # Check if file already exists
        if filename in self.file_system_nodes:
            print(f"File {filename} already exists")
            return False
        
        # Create new file node
        new_file = FileSystemNode(
            name=filename,
            node_type="file",
            children={},
            size=size
        ...
        self.file_system_nodes[filename] = new_file
        self.parent_map[filename] = folder_name
        
        # Add to folder's children (update the raw JSON data)
        folder_node.children[filename] = {
            "type": "file",
            "size": size
        }
        
        print(f"Added {filename} to {folder_path}")
        return True